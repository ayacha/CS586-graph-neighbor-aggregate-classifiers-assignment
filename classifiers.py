...
class LocalClassifier(Classifier):
    ...
    def fit(self, graph, train_indices):
        X = [graph.node_list[i].feature_vector for i in train_indices]
        y = [graph.node_list[i].label for i in train_indices]
        self.clf.fit(X,y)


    def predict(self, graph, test_indices, conditional_node_to_label_map = None):
        X = [graph.node_list[i].feature_vector for i in test_indices]
